...
# Bob gen keys
public_key, private_key = rsa.newkeys(600)
...
dig_sign = rsa.encrypt(msg_hash, public_key)
...
ALICE_MSG = {'text': encrypted.hex(), 'dig_sign': dig_sign.hex(),
             'sym_key': sym_key_encrypted.hex(), 'nonce': cipher.nonce.hex()}
print(ALICE_MSG)

# Bob gets
del message, name, msg, salt, msg_hash,\
...
cipher = AES.new(sym_key, AES.MODE_EAX, nonce=bytes.fromhex(nonce))
text = cipher.decrypt(text)
print('encoded msg:', text)
text = text.decode()
dig_sign = bytes.fromhex(dig_sign)
dig_sign = rsa.decrypt(dig_sign, private_key)
corrupted = not bcrypt.checkpw(text.encode(), dig_sign)
